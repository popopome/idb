...
(function (root) {
    ...
    function success_error_to_ob(req, is_loop_request) {
    ...
    function count(ctx) {
        var req = get_store_rw(ctx.db).count();
        return success_error_to_ob(req)
            .select(function(result) {
                ctx.result = result;
                return ctx;
    ...
    /**
     * set has two steps.
     * - 1st step: save key and value as actual format
     * - 2nd step: save timestamp with key value.
     *
     * @param ctx
     * @param k
     * @param v
     * @returns {Observable|*|selectMany|selectMany}
     */
    function set(ctx, k, v) {

        var item = kv(k, v);
        item[TIMESTAMP_KEY] = now_utc();
    ...
    function all(ctx) {
        var req = get_store_ro(ctx.db).openCursor();
        return success_error_to_ob(req, true)
            .select(function (c) {
                ctx.result = c.value;
                c.continue();
                return ctx;
    ...
    root.idb.open = open;
    root.idb.clear = clear;
    root.idb.count = count;
    root.idb.set = set;
    root.idb.get = get;
    root.idb.all = all;
